...
const ISRAEL_BOUNDS = [[28.0, 33.5], [34.5, 37.2]];
...
function stopGPS() {
  ...
  if (gpsMarker) { mainMap.removeLayer(gpsMarker); gpsMarker = null; }
  if (gpsAccuracyCircle) { mainMap.removeLayer(gpsAccuracyCircle); gpsAccuracyCircle = null; }
  const btn = document.getElementById('btn-gps');
...
function updateGPSMarker(lat, lng, accuracy) {
  if (gpsMarker) mainMap.removeLayer(gpsMarker);
  if (gpsAccuracyCircle) mainMap.removeLayer(gpsAccuracyCircle);

  if (accuracy > 30) {
    gpsAccuracyCircle = L.circle([lat, lng], {
      radius: accuracy, fillColor: '#2196F3', fillOpacity: 0.1,
